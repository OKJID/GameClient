...
void LANAPI::sendMessage(LANMessage* msg, UnsignedInt ip /* = 0 */)
{
	...
	if (ip != 0)
	{
		m_transport->queueSend(ip, lobbyPort, payload, payloadSize /*, 0, 0 */);
	}
		...
		for (Int i = 0; i < MAX_SLOTS; ++i)
		{
			if (i != localSlot) {
				...
				if ((slot != nullptr) && (slot->isHuman())) {
					m_transport->queueSend(slot->getIP(), lobbyPort, payload, payloadSize /*, 0, 0 */);
				}
			}
		}
	}
	...
	{
		m_transport->queueSend(m_broadcastAddr, lobbyPort, payload, payloadSize /*, 0, 0 */);
	}
}
