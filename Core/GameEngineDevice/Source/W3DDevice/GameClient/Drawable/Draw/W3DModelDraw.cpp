...
void ModelConditionInfo::validateCachedBones(RenderObjClass* robj, Real scale) const
{
	...
	if (TheGameLogic)
	{
		...
		if (s_diagFile)
		{
			...
			{
				fprintf(s_diagFile, "  BONE hash=%u str='%s' idx=%d x=%f y=%f z=%f\n", 
					it->first,
...
{
	AnimParseType animType = (AnimParseType)(size_t)userData;
...
void W3DModelDrawModuleData::parseConditionState(INI* ini, void *instance, void * /*store*/, const void* userData)
{
	...
	ParseCondStateType cst = (ParseCondStateType)(size_t)userData;
	switch (cst)
	{
		case PARSE_DEFAULT:
		{
